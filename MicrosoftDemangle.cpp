...
static bool startsWithDigit(StringView S) { return !S.empty() && is_digit(S.front()); }
...
enum NameBackrefBehavior : uint8 {
...
static bool isMemberPointer(StringView MangledName) {
	switch (MangledName.popFront()) {
		...
		case 'S':
			...
			// what.
			break;
		default: assert(false && "Ty is not a pointer type!");
	}
...
static bool isFunctionType(StringView S) { return S.startsWith("$$A8@@") || S.startsWith("$$A6"); }
...
static pair<Qualifiers, PointerAffinity> demanglePointerCVQualifiers(StringView& MangledName) {
	if (MangledName.consumeFront("$$Q"))
		return make_pair(Q_None, PointerAffinity::RValueReference);

	switch (MangledName.popFront()) {
		case 'A': return make_pair(Q_None, PointerAffinity::Reference);
		case 'P': return make_pair(Q_None, PointerAffinity::Pointer);
		case 'Q': return make_pair(Q_Const, PointerAffinity::Pointer);
		case 'R': return make_pair(Q_Volatile, PointerAffinity::Pointer);
		case 'S': return make_pair(Qualifiers(Q_Const | Q_Volatile), PointerAffinity::Pointer);
		default: assert(false && "Ty is not a pointer type!");
	}
	return make_pair(Q_None, PointerAffinity::Pointer);
}
...
private:
	SymbolNode* demangleEncodedSymbol(StringView& MangledName, QualifiedNameNode* QN);

	VariableSymbolNode* demangleVariableEncoding(StringView& MangledName, StorageClass SC);
	...
	FunctionSignatureNode* demangleFunctionType(StringView& MangledName, bool HasThisQuals);
	...
	pair<uint64, bool> demangleNumber(StringView& MangledName);
	uint64				  demangleUnsigned(StringView& MangledName);
	int64					  demangleSigned(StringView& MangledName);
	...
	IdentifierNode* demangleUnqualifiedTypeName(StringView& MangledName, bool Memorize);
	IdentifierNode* demangleUnqualifiedSymbolName(StringView& MangledName, NameBackrefBehavior NBB);

	QualifiedNameNode* demangleNameScopeChain(StringView& MangledName, IdentifierNode* UnqualifiedName);
	IdentifierNode*	demangleNameScopePiece(StringView& MangledName);

	NamedIdentifierNode*			  demangleBackRefName(StringView& MangledName);
	IdentifierNode*					  demangleTemplateInstantiationName(StringView& MangledName, NameBackrefBehavior NBB);
	IdentifierNode*					  demangleFunctionIdentifierCode(StringView& MangledName);
	IdentifierNode*					  demangleFunctionIdentifierCode(StringView& MangledName, FunctionIdentifierCodeGroup Group);
	StructorIdentifierNode*			  demangleStructorIdentifier(StringView& MangledName, bool IsDestructor);
	ConversionOperatorIdentifierNode* demangleConversionOperatorIdentifier(StringView& MangledName);
	...
	SpecialTableSymbolNode*		  demangleSpecialTableSymbolNode(StringView& MangledName, SpecialIntrinsicKind SIK);
	LocalStaticGuardVariableNode* demangleLocalStaticGuard(StringView& MangledName);
	VariableSymbolNode*			  demangleUntypedVariable(ArenaAllocator& Arena, StringView& MangledName, StringView VariableName);
	VariableSymbolNode*			  demangleRttiBaseClassDescriptorNode(ArenaAllocator& Arena, StringView& MangledName);
	FunctionSymbolNode*			  demangleInitFiniStub(StringView& MangledName, bool IsDestructor);

	NamedIdentifierNode*	  demangleSimpleName(StringView& MangledName, bool Memorize);
	...
	uint8		 demangleCharLiteral(StringView& MangledName);

	pair<Qualifiers, bool> demangleQualifiers(StringView& MangledName);
...
StringView Demangler::copyString(StringView Borrowed) {
	...
	strcpy(Stable, Borrowed.begin());

	return {Stable, Borrowed.size()};
}

SpecialTableSymbolNode* Demangler::demangleSpecialTableSymbolNode(StringView& MangledName, SpecialIntrinsicKind K) {
	NamedIdentifierNode* NI = Arena.alloc<NamedIdentifierNode>();
	switch (K) {
		case SpecialIntrinsicKind::Vftable: NI->Name = "`vftable'"; break;
		case SpecialIntrinsicKind::Vbtable: NI->Name = "`vbtable'"; break;
		case SpecialIntrinsicKind::LocalVftable: NI->Name = "`local vftable'"; break;
		case SpecialIntrinsicKind::RttiCompleteObjLocator: NI->Name = "`RTTI Complete Object Locator'"; break;
		default: unreachable();
	}
	...
	tie(STSN->Quals, IsMember) = demangleQualifiers(MangledName);
	if (!MangledName.consumeFront('@'))
		STSN->TargetName = demangleFullyQualifiedTypeName(MangledName);
	return STSN;
}

LocalStaticGuardVariableNode* Demangler::demangleLocalStaticGuard(StringView& MangledName) {
	LocalStaticGuardIdentifierNode* LSGI  = Arena.alloc<LocalStaticGuardIdentifierNode>();
	QualifiedNameNode*				QN	= demangleNameScopeChain(MangledName, LSGI);
	LocalStaticGuardVariableNode*   LSGVN = Arena.alloc<LocalStaticGuardVariableNode>();
	LSGVN->Name							  = QN;
...
static QualifiedNameNode* synthesizeQualifiedName(ArenaAllocator& Arena, StringView Name) {
...
VariableSymbolNode* Demangler::demangleUntypedVariable(ArenaAllocator& Arena, StringView& MangledName, StringView VariableName) {
...
VariableSymbolNode* Demangler::demangleRttiBaseClassDescriptorNode(ArenaAllocator& Arena, StringView& MangledName) {
...
FunctionSymbolNode* Demangler::demangleInitFiniStub(StringView& MangledName, bool IsDestructor) {
	DynamicStructorIdentifierNode* DSIN = Arena.alloc<DynamicStructorIdentifierNode>();
...
SymbolNode* Demangler::demangleSpecialIntrinsic(StringView& MangledName) {
	...
	switch (SIK) {
		case SpecialIntrinsicKind::StringLiteralSymbol: return demangleStringLiteral(MangledName);
		case SpecialIntrinsicKind::Vftable:
		case SpecialIntrinsicKind::Vbtable:
		case SpecialIntrinsicKind::LocalVftable:
		case SpecialIntrinsicKind::RttiCompleteObjLocator: return demangleSpecialTableSymbolNode(MangledName, SIK);
		case SpecialIntrinsicKind::VcallThunk: return demangleVcallThunkNode(MangledName);
		case SpecialIntrinsicKind::LocalStaticGuard: return demangleLocalStaticGuard(MangledName);
		...
		case SpecialIntrinsicKind::RttiBaseClassArray: return demangleUntypedVariable(Arena, MangledName, "`RTTI Base Class Array'");
		case SpecialIntrinsicKind::RttiClassHierarchyDescriptor: return demangleUntypedVariable(Arena, MangledName, "`RTTI Class Hierarchy Descriptor'");
		case SpecialIntrinsicKind::RttiBaseClassDescriptor: return demangleRttiBaseClassDescriptorNode(Arena, MangledName);
		case SpecialIntrinsicKind::DynamicInitializer: return demangleInitFiniStub(MangledName, false);
		case SpecialIntrinsicKind::DynamicAtexitDestructor: return demangleInitFiniStub(MangledName, true);
		default: break;
	}
	Error = true;
	return nullptr;
...
IdentifierNode* Demangler::demangleFunctionIdentifierCode(StringView& MangledName) {
	assert(MangledName.startsWith('?'));
	MangledName = MangledName.dropFront();

	if (MangledName.consumeFront("__"))
		return demangleFunctionIdentifierCode(MangledName, FunctionIdentifierCodeGroup::DoubleUnder);
	else if (MangledName.consumeFront("_"))
		return demangleFunctionIdentifierCode(MangledName, FunctionIdentifierCodeGroup::Under);
	return demangleFunctionIdentifierCode(MangledName, FunctionIdentifierCodeGroup::Basic);
}

StructorIdentifierNode* Demangler::demangleStructorIdentifier(StringView& MangledName, bool IsDestructor) {
	StructorIdentifierNode* N = Arena.alloc<StructorIdentifierNode>();
	N->IsDestructor			  = IsDestructor;
	return N;
}

ConversionOperatorIdentifierNode* Demangler::demangleConversionOperatorIdentifier(StringView& MangledName) {
	ConversionOperatorIdentifierNode* N = Arena.alloc<ConversionOperatorIdentifierNode>();
	return N;
}

LiteralOperatorIdentifierNode* Demangler::demangleLiteralOperatorIdentifier(StringView& MangledName) {
	LiteralOperatorIdentifierNode* N = Arena.alloc<LiteralOperatorIdentifierNode>();
...
IntrinsicFunctionKind translateIntrinsicFunctionCode(char CH, FunctionIdentifierCodeGroup Group) {
	...
	switch (Group) {
		case FunctionIdentifierCodeGroup::Basic: return Basic[Index];
		case FunctionIdentifierCodeGroup::Under: return Under[Index];
		case FunctionIdentifierCodeGroup::DoubleUnder: return DoubleUnder[Index];
	}
	unreachable();
}

IdentifierNode* Demangler::demangleFunctionIdentifierCode(StringView& MangledName, FunctionIdentifierCodeGroup Group) {
	switch (Group) {
		case FunctionIdentifierCodeGroup::Basic:
			switch (char CH = MangledName.popFront()) {
				case '0':
				case '1': return demangleStructorIdentifier(MangledName, CH == '1');
				case 'B': return demangleConversionOperatorIdentifier(MangledName);
				default: return Arena.alloc<IntrinsicFunctionIdentifierNode>(translateIntrinsicFunctionCode(CH, Group));
			}
			break;
		case FunctionIdentifierCodeGroup::Under: return Arena.alloc<IntrinsicFunctionIdentifierNode>(translateIntrinsicFunctionCode(MangledName.popFront(), Group));
		case FunctionIdentifierCodeGroup::DoubleUnder:
			switch (char CH = MangledName.popFront()) {
				case 'K': return demangleLiteralOperatorIdentifier(MangledName);
				default: return Arena.alloc<IntrinsicFunctionIdentifierNode>(translateIntrinsicFunctionCode(CH, Group));
			}
	}
...
SymbolNode* Demangler::demangleEncodedSymbol(StringView& MangledName, QualifiedNameNode* Name) {
	// Read a variable.
	switch (MangledName.front()) {
		case '0':
		case '1':
	...
	if (UQN->kind() == NodeKind::ConversionOperatorIdentifier) {
		ConversionOperatorIdentifierNode* COIN = static_cast<ConversionOperatorIdentifierNode*>(UQN);
...
VariableSymbolNode* Demangler::demangleVariableEncoding(StringView& MangledName, StorageClass SC) {
	VariableSymbolNode* VSN = Arena.alloc<VariableSymbolNode>();
	...
	switch (VSN->Type->kind()) {
		case NodeKind::PointerType: {
			...
			PTN->Quals				   = Qualifiers(VSN->Type->Quals | demanglePointerExtQualifiers(MangledName));

			bool IsMember						= false;
			tie(ExtraChildQuals, IsMember) = demangleQualifiers(MangledName);

			if (PTN->ClassParent) {
				QualifiedNameNode* BackRefName = demangleFullyQualifiedTypeName(MangledName);
				(void)BackRefName;
			}
		...
		default: VSN->Type->Quals = demangleQualifiers(MangledName).a; break;
	}
...
// "int (*x)[20]" is a valid C type (x is a pointer to an fixed_array of
...
pair<uint64, bool> Demangler::demangleNumber(StringView& MangledName) {
	bool IsNegative = MangledName.consumeFront('?');

	if (startsWithDigit(MangledName)) {
		uint64 Ret = MangledName[0] - '0' + 1;
...
uint64 Demangler::demangleUnsigned(StringView& MangledName) {
	bool	 IsNegative			 = false;
	uint64 Number				 = 0;
	tie(Number, IsNegative) = demangleNumber(MangledName);
	if (IsNegative)
		Error = true;
	return Number;
}

int64 Demangler::demangleSigned(StringView& MangledName) {
	bool	 IsNegative			 = false;
	uint64 Number				 = 0;
	tie(Number, IsNegative) = demangleNumber(MangledName);
	if (Number > (int64)maximum)
		Error = true;
	int64 I = static_cast<int64>(Number);
...
void Demangler::memorizeIdentifier(IdentifierNode* Identifier) {
	...
	StringView Owned = copyString(OS.term());
	memorizeString(Owned);
}

IdentifierNode* Demangler::demangleTemplateInstantiationName(StringView& MangledName, NameBackrefBehavior NBB) {
	assert(MangledName.startsWith("?$"));
	MangledName.consumeFront("?$");
...
NamedIdentifierNode* Demangler::demangleSimpleName(StringView& MangledName, bool Memorize) {
...
static uint8 rebasedHexDigitToNumber(char C) {
	assert(isRebasedHexDigit(C));
	return (C <= 'J') ? (C - 'A') : (10 + C - 'K');
}

uint8 Demangler::demangleCharLiteral(StringView& MangledName) {
	if (!MangledName.startsWith('?'))
	...
	if (MangledName.consumeFront('$')) {
		...
		uint8 C1  = rebasedHexDigitToNumber(Nibbles[0]);
		uint8 C2  = rebasedHexDigitToNumber(Nibbles[1]);
	...
	if (MangledName[0] >= 'a' && MangledName[0] <= 'z') {
		char Lookup[26] = {'\xE1', '\xE2', '\xE3', '\xE4', '\xE5', '\xE6', '\xE7', '\xE8', '\xE9', '\xEA', '\xEB', '\xEC', '\xED', '\xEE', '\xEF', '\xF0', '\xF1', '\xF2', '\xF3', '\xF4', '\xF5', '\xF6', '\xF7', '\xF8', '\xF9', '\xFA'};
	...
	if (MangledName[0] >= 'A' && MangledName[0] <= 'Z') {
		char Lookup[26] = {'\xC1', '\xC2', '\xC3', '\xC4', '\xC5', '\xC6', '\xC7', '\xC8', '\xC9', '\xCA', '\xCB', '\xCC', '\xCD', '\xCE', '\xCF', '\xD0', '\xD1', '\xD2', '\xD3', '\xD4', '\xD5', '\xD6', '\xD7', '\xD8', '\xD9', '\xDA'};
		char C			= Lookup[MangledName[0] - 'A'];
		MangledName		= MangledName.dropFront();
		return C;
	}
...
static void writeHexDigit(char* Buffer, uint8 Digit) {
	assert(Digit <= 15);
	*Buffer = (Digit < 10) ? ('0' + Digit) : ('A' + Digit - 10);
}

static void outputHex(string_accum& OS, unsigned C) {
	if (C == 0) {
		OS << "\\x00";
		return;
	}
...
static void outputEscapedChar(string_accum& OS, unsigned C) {
	switch (C) {
		case '\'':  // single quote
			OS << "\\\'";
			return;
...
unsigned countTrailingNullBytes(const uint8* StringBytes, int Length) {
	const uint8* End   = StringBytes + Length - 1;
	unsigned	   Count = 0;
...
unsigned countEmbeddedNulls(const uint8* StringBytes, unsigned Length) {
	unsigned Result = 0;
...
unsigned guessCharByteSize(const uint8* StringBytes, unsigned NumChars, unsigned NumBytes) {
	assert(NumBytes > 0);
...
static unsigned decodeMultiByteChar(const uint8* StringBytes, unsigned CharIndex, unsigned CharBytes) {
...
EncodedStringLiteralNode* Demangler::demangleStringLiteral(StringView& MangledName) {
	...
	switch (MangledName.popFront()) {
		case '1': IsWcharT = true; LLVM_FALLTHROUGH;
		case '0': break;
		default: goto StringLiteralError;
	}

	// Encoded Length
	tie(StringByteSize, IsNegative) = demangleNumber(MangledName);
	if (Error || IsNegative)
		goto StringLiteralError;
	...
	if (IsWcharT) {
		...
		uint8			   StringBytes[MaxStringByteLength];
		...
		switch (CharBytes) {
			...
			case 2: Result->Char = CharKind::Char16; break;
			case 4: Result->Char = CharKind::Char32; break;
			default: unreachable();
		}
		const unsigned NumChars = BytesDecoded / CharBytes;
		for (unsigned CharIndex = 0; CharIndex < NumChars; ++CharIndex) {
			unsigned NextChar = decodeMultiByteChar(StringBytes, CharIndex, CharBytes);
			if (CharIndex + 1 < NumChars || Result->IsTruncated)
				outputEscapedChar(OS, NextChar);
		}
	}

	Result->DecodedString = copyString(OS.term());
	return Result;
...
StringView Demangler::demangleSimpleString(StringView& MangledName, bool Memorize) {
	StringView S;
	for (size_t i = 0; i < MangledName.size(); ++i) {
		if (MangledName[i] != '@')
			continue;
...
NamedIdentifierNode* Demangler::demangleAnonymousNamespaceName(StringView& MangledName) {
...
NamedIdentifierNode* Demangler::demangleLocallyScopedNamePiece(StringView& MangledName) {
	...
	OS << "::`" << Number.a << "'";
	OS << '\0';
	Identifier->Name = copyString(OS.term());
	return Identifier;
}

// Parses a type name in the form of A@B@C@@ which represents C::B::A.
QualifiedNameNode* Demangler::demangleFullyQualifiedTypeName(StringView& MangledName) {
...
QualifiedNameNode* Demangler::demangleFullyQualifiedSymbolName(StringView& MangledName) {
	...
	if (Identifier->kind() == NodeKind::StructorIdentifier) {
		StructorIdentifierNode* SIN = static_cast<StructorIdentifierNode*>(Identifier);
...
IdentifierNode* Demangler::demangleUnqualifiedTypeName(StringView& MangledName, bool Memorize) {
...
IdentifierNode* Demangler::demangleUnqualifiedSymbolName(StringView& MangledName, NameBackrefBehavior NBB) {
	if (startsWithDigit(MangledName))
		return demangleBackRefName(MangledName);
	if (MangledName.startsWith("?$"))
...
QualifiedNameNode* Demangler::demangleNameScopeChain(StringView& MangledName, IdentifierNode* UnqualifiedName) {
...
FuncClass Demangler::demangleFunctionClass(StringView& MangledName) {
	switch (MangledName.popFront()) {
		case '9': return FuncClass(FC_ExternC | FC_NoParameterList);
		case 'A': return FC_Private;
		case 'B': return FuncClass(FC_Private | FC_Far);
		case 'C': return FuncClass(FC_Private | FC_Static);
		case 'D': return FuncClass(FC_Private | FC_Static);
		case 'E': return FuncClass(FC_Private | FC_Virtual);
		case 'F': return FuncClass(FC_Private | FC_Virtual);
		case 'G': return FuncClass(FC_Private | FC_StaticThisAdjust);
		case 'H': return FuncClass(FC_Private | FC_StaticThisAdjust | FC_Far);
		case 'I': return FuncClass(FC_Protected);
		case 'J': return FuncClass(FC_Protected | FC_Far);
		case 'K': return FuncClass(FC_Protected | FC_Static);
		case 'L': return FuncClass(FC_Protected | FC_Static | FC_Far);
		case 'M': return FuncClass(FC_Protected | FC_Virtual);
		case 'N': return FuncClass(FC_Protected | FC_Virtual | FC_Far);
		case 'O': return FuncClass(FC_Protected | FC_Virtual | FC_StaticThisAdjust);
		case 'P': return FuncClass(FC_Protected | FC_Virtual | FC_StaticThisAdjust | FC_Far);
		case 'Q': return FuncClass(FC_Public);
		case 'R': return FuncClass(FC_Public | FC_Far);
		case 'S': return FuncClass(FC_Public | FC_Static);
		case 'T': return FuncClass(FC_Public | FC_Static | FC_Far);
		case 'U': return FuncClass(FC_Public | FC_Virtual);
		case 'V': return FuncClass(FC_Public | FC_Virtual | FC_Far);
		case 'W': return FuncClass(FC_Public | FC_Virtual | FC_StaticThisAdjust);
		case 'X': return FuncClass(FC_Public | FC_Virtual | FC_StaticThisAdjust | FC_Far);
		case 'Y': return FuncClass(FC_Global);
		case 'Z': return FuncClass(FC_Global | FC_Far);
		case '$': {
			...
			switch (MangledName.popFront()) {
				case '0': return FuncClass(FC_Private | FC_Virtual | VFlag);
				case '1': return FuncClass(FC_Private | FC_Virtual | VFlag | FC_Far);
				case '2': return FuncClass(FC_Protected | FC_Virtual | VFlag);
				case '3': return FuncClass(FC_Protected | FC_Virtual | VFlag | FC_Far);
				case '4': return FuncClass(FC_Public | FC_Virtual | VFlag);
				case '5': return FuncClass(FC_Public | FC_Virtual | VFlag | FC_Far);
			}
		}
	}
...
CallingConv Demangler::demangleCallingConvention(StringView& MangledName) {
	switch (MangledName.popFront()) {
		case 'A':
		case 'B': return CallingConv::Cdecl;
		case 'C':
		case 'D': return CallingConv::Pascal;
		case 'E':
		case 'F': return CallingConv::Thiscall;
		case 'G':
		case 'H': return CallingConv::Stdcall;
		case 'I':
		case 'J': return CallingConv::Fastcall;
		case 'M':
		case 'N': return CallingConv::Clrcall;
		case 'O':
		case 'P': return CallingConv::Eabi;
...
StorageClass Demangler::demangleVariableStorageClass(StringView& MangledName) {
	assert(isdigit(MangledName.front()));

	switch (MangledName.popFront()) {
		case '0': return StorageClass::PrivateStatic;
		case '1': return StorageClass::ProtectedStatic;
		case '2': return StorageClass::PublicStatic;
		case '3': return StorageClass::Global;
		case '4': return StorageClass::FunctionLocalStatic;
	}
	Error = true;
	return StorageClass::None;
}

pair<Qualifiers, bool> Demangler::demangleQualifiers(StringView& MangledName) {
	switch (MangledName.popFront()) {
		// Member qualifiers
		case 'Q': return make_pair(Q_None, true);
		case 'R': return make_pair(Q_Const, true);
		case 'S': return make_pair(Q_Volatile, true);
		case 'T': return make_pair(Qualifiers(Q_Const | Q_Volatile), true);
		// Non-Member qualifiers
		case 'A': return make_pair(Q_None, false);
		case 'B': return make_pair(Q_Const, false);
		case 'C': return make_pair(Q_Volatile, false);
		case 'D': return make_pair(Qualifiers(Q_Const | Q_Volatile), false);
	}
	Error = true;
	return make_pair(Q_None, false);
}

// <variable-type> ::= <type> <cvr-qualifiers>
//                 ::= <type> <pointee-cvr-qualifiers> # pointers, references
TypeNode* Demangler::demangleType(StringView& MangledName, QualifierMangleMode QMM) {
	Qualifiers Quals	= Q_None;
	bool	   IsMember = false;
	if (QMM == QualifierMangleMode::Mangle) {
		tie(Quals, IsMember) = demangleQualifiers(MangledName);
...
FunctionSignatureNode* Demangler::demangleFunctionType(StringView& MangledName, bool HasThisQuals) {
	...
	if (HasThisQuals) {
		...
		FTy->Quals		  = Qualifiers(FTy->Quals | demangleQualifiers(MangledName).a);
	}
...
FunctionSymbolNode* Demangler::demangleFunctionEncoding(StringView& MangledName) {
...
PrimitiveTypeNode* Demangler::demanglePrimitiveType(StringView& MangledName) {
	...
	switch (MangledName.popFront()) {
		case 'X': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Void);
		case 'D': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Char);
		case 'C': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Schar);
		case 'E': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Uchar);
		case 'F': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Short);
		case 'G': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Ushort);
		case 'H': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Int);
		case 'I': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Uint);
		case 'J': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Long);
		case 'K': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Ulong);
		case 'M': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Float);
		case 'N': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Double);
		case 'O': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Ldouble);
		case '_': {
			if (MangledName.empty()) {
				Error = true;
				return nullptr;
			...
			switch (MangledName.popFront()) {
				case 'N': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Bool);
				case 'J': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Int64);
				case 'K': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Uint64);
				case 'W': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Wchar);
				case 'S': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Char16);
				case 'U': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Char32);
			}
			break;
		}
	}
...
TagTypeNode* Demangler::demangleClassType(StringView& MangledName) {
	...
	switch (MangledName.popFront()) {
		case 'T': TT = Arena.alloc<TagTypeNode>(TagKind::Union); break;
		case 'U': TT = Arena.alloc<TagTypeNode>(TagKind::Struct); break;
		case 'V': TT = Arena.alloc<TagTypeNode>(TagKind::Class); break;
		case 'W':
...
PointerTypeNode* Demangler::demanglePointerType(StringView& MangledName) {
	...
	tie(Pointer->Quals, Pointer->Affinity) = demanglePointerCVQualifiers(MangledName);
...
PointerTypeNode* Demangler::demangleMemberPointerType(StringView& MangledName) {
	...
	if (MangledName.consumeFront("8")) {
		...
		bool	   IsMember				 = false;
		tie(PointeeQuals, IsMember) = demangleQualifiers(MangledName);
		assert(IsMember);
		Pointer->ClassParent = demangleFullyQualifiedTypeName(MangledName);
...
ArrayTypeNode* Demangler::demangleArrayType(StringView& MangledName) {
	...
	uint64 Rank			   = 0;
	bool	 IsNegative		   = false;
	tie(Rank, IsNegative) = demangleNumber(MangledName);
	...
	for (uint64 I = 0; I < Rank; ++I) {
		uint64 D				= 0;
		tie(D, IsNegative) = demangleNumber(MangledName);
		if (IsNegative) {
			Error = true;
			return nullptr;
		}
	...
	if (MangledName.consumeFront("$$C")) {
		bool IsMember				   = false;
		tie(ATy->Quals, IsMember) = demangleQualifiers(MangledName);
		if (IsMember) {
			Error = true;
			return nullptr;
		}
...
NodeArrayNode* Demangler::demangleTemplateParameterList(StringView& MangledName) {
	NodeList*  Head;
	NodeList** Current = &Head;
	size_t	 Count   = 0;

	while (!Error && !MangledName.startsWith('@')) {
		if (MangledName.consumeFront("$S") || MangledName.consumeFront("$$V") || MangledName.consumeFront("$$$V") || MangledName.consumeFront("$$Z")) {
		...
		*Current = Arena.alloc<NodeList>();
		...
		if (MangledName.consumeFront("$$Y")) {
		...
		} else if (MangledName.startsWith("$1") || MangledName.startsWith("$H") || MangledName.startsWith("$I") || MangledName.startsWith("$J")) {
			// Pointer to member
			...
			switch (InheritanceSpecifier) {
				case 'J': TPRN->ThunkOffsets[TPRN->ThunkOffsetCount++] = demangleSigned(MangledName); LLVM_FALLTHROUGH;
				case 'I': TPRN->ThunkOffsets[TPRN->ThunkOffsetCount++] = demangleSigned(MangledName); LLVM_FALLTHROUGH;
				case 'H': TPRN->ThunkOffsets[TPRN->ThunkOffsetCount++] = demangleSigned(MangledName); LLVM_FALLTHROUGH;
				case '1': break;
			...
			switch (InheritanceSpecifier) {
				case 'G': TPRN->ThunkOffsets[TPRN->ThunkOffsetCount++] = demangleSigned(MangledName); LLVM_FALLTHROUGH;
				case 'F':
			...
			bool	 IsNegative			= false;
			uint64 Value				= 0;
			tie(Value, IsNegative) = demangleNumber(MangledName);
...
string microsoftDemangle(const char* MangledName) {
	Demangler	D;
