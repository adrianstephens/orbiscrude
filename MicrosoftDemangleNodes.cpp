...
static void outputSpaceIfNecessary(string_accum& OS) {
	char C = OS.back();
	if (is_alphanum(C) || C == '>')
		OS << " ";
}

static bool outputSingleQualifier(string_accum& OS, Qualifiers Q) {
	switch (Q) {
		case Q_Const: OS << "const"; return true;
		case Q_Volatile: OS << "volatile"; return true;
		case Q_Restrict: OS << "__restrict"; return true;
		default: break;
	}
	return false;
}

static bool outputQualifierIfPresent(string_accum& OS, Qualifiers Q, Qualifiers Mask, bool NeedSpace) {
	if (!(Q & Mask))
		return NeedSpace;
...
static void outputQualifiers(string_accum& OS, Qualifiers Q, bool SpaceBefore, bool SpaceAfter) {
	if (Q == Q_None)
		return;
...
static void outputCallingConvention(string_accum& OS, CallingConv CC) {
...
void PrimitiveTypeNode::outputPre(string_accum& OS, OutputFlags Flags) const {
	switch (PrimKind) {
		...
		OUTPUT_ENUM_CLASS_VALUE(PrimitiveKind, Nullptr, "nullptr_t");
	}
	outputQualifiers(OS, Quals, true, false);
}

void NodeArrayNode::output(string_accum& OS, OutputFlags Flags) const { output(OS, Flags, ", "); }

void NodeArrayNode::output(string_accum& OS, OutputFlags Flags, StringView Separator) const {
	if (Count == 0)
		return;
	if (Nodes[0])
...
void EncodedStringLiteralNode::output(string_accum& OS, OutputFlags Flags) const {
	switch (Char) {
		case CharKind::Wchar: OS << "const wchar_t * {L\""; break;
		case CharKind::Char: OS << "const char * {\""; break;
		case CharKind::Char16: OS << "const char16_t * {u\""; break;
		case CharKind::Char32: OS << "const char32_t * {U\""; break;
	}
	OS << DecodedString << "\"";
	if (IsTruncated)
		OS << "...";
...
void IntegerLiteralNode::output(string_accum& OS, OutputFlags Flags) const {
	if (IsNegative)
		OS << '-';
	OS << Value;
}

void TemplateParameterReferenceNode::output(string_accum& OS, OutputFlags Flags) const {
...
void IdentifierNode::outputTemplateParameters(string_accum& OS, OutputFlags Flags) const {
	if (!TemplateParams)
		return;
	OS << "<";
...
void DynamicStructorIdentifierNode::output(string_accum& OS, OutputFlags Flags) const {
	if (IsDestructor)
...
void NamedIdentifierNode::output(string_accum& OS, OutputFlags Flags) const {
	OS << Name;
	outputTemplateParameters(OS, Flags);
}

void IntrinsicFunctionIdentifierNode::output(string_accum& OS, OutputFlags Flags) const {
	switch (Operator) {
		...
		OUTPUT_ENUM_CLASS_VALUE(IntrinsicFunctionKind, ManVectorVbaseCopyCtorIter, "`managed vector vbase copy constructor iterator'");
...
void LocalStaticGuardIdentifierNode::output(string_accum& OS, OutputFlags Flags) const {
	OS << "`local static guard'";
	if (ScopeIndex > 0)
		OS << "{" << ScopeIndex << "}";
}

void ConversionOperatorIdentifierNode::output(string_accum& OS, OutputFlags Flags) const {
	OS << "operator";
	outputTemplateParameters(OS, Flags);
	OS << " ";
...
void StructorIdentifierNode::output(string_accum& OS, OutputFlags Flags) const {
	if (IsDestructor)
		OS << "~";
...
void LiteralOperatorIdentifierNode::output(string_accum& OS, OutputFlags Flags) const {
	OS << "operator \"\"" << Name;
	outputTemplateParameters(OS, Flags);
}

void FunctionSignatureNode::outputPre(string_accum& OS, OutputFlags Flags) const {
...
void FunctionSignatureNode::outputPost(string_accum& OS, OutputFlags Flags) const {
...
void ThunkSignatureNode::outputPre(string_accum& OS, OutputFlags Flags) const {
	OS << "[thunk]: ";

	FunctionSignatureNode::outputPre(OS, Flags);
}

void ThunkSignatureNode::outputPost(string_accum& OS, OutputFlags Flags) const {
	if (FunctionClass & FC_StaticThisAdjust) {
		OS << "`adjustor{" << ThisAdjust.StaticOffset << "}'";
	} else if (FunctionClass & FC_VirtualThisAdjust) {
		if (FunctionClass & FC_VirtualThisAdjustEx) {
			OS << "`vtordispex{" << ThisAdjust.VBPtrOffset << ", " << ThisAdjust.VBOffsetOffset << ", " << ThisAdjust.VtordispOffset << ", " << ThisAdjust.StaticOffset << "}'";
		} else {
...
void PointerTypeNode::outputPre(string_accum& OS, OutputFlags Flags) const {
...
void PointerTypeNode::outputPost(string_accum& OS, OutputFlags Flags) const {
...
void TagTypeNode::outputPre(string_accum& OS, OutputFlags Flags) const {
	switch (Tag) {
...
void TagTypeNode::outputPost(string_accum& OS, OutputFlags Flags) const {}

void ArrayTypeNode::outputPre(string_accum& OS, OutputFlags Flags) const {
	ElementType->outputPre(OS, Flags);
	outputQualifiers(OS, Quals, true, false);
}

void ArrayTypeNode::outputOneDimension(string_accum& OS, OutputFlags Flags, Node* N) const {
...
void ArrayTypeNode::outputDimensionsImpl(string_accum& OS, OutputFlags Flags) const {
...
void ArrayTypeNode::outputPost(string_accum& OS, OutputFlags Flags) const {
	OS << "[";
...
void SymbolNode::output(string_accum& OS, OutputFlags Flags) const { Name->output(OS, Flags); }

void FunctionSymbolNode::output(string_accum& OS, OutputFlags Flags) const {
...
void VariableSymbolNode::output(string_accum& OS, OutputFlags Flags) const {
	switch (SC) {
		case StorageClass::PrivateStatic:
		case StorageClass::PublicStatic:
		case StorageClass::ProtectedStatic: OS << "static ";
		default: break;
	}
...
void CustomTypeNode::outputPre(string_accum& OS, OutputFlags Flags) const { Identifier->output(OS, Flags); }
void CustomTypeNode::outputPost(string_accum& OS, OutputFlags Flags) const {}

void QualifiedNameNode::output(string_accum& OS, OutputFlags Flags) const { Components->output(OS, Flags, "::"); }

void RttiBaseClassDescriptorNode::output(string_accum& OS, OutputFlags Flags) const {
	OS << "`RTTI Base Class Descriptor at (";
	OS << NVOffset << ", " << VBPtrOffset << ", " << VBTableOffset << ", " << this->Flags;
	OS << ")'";
}

void LocalStaticGuardVariableNode::output(string_accum& OS, OutputFlags Flags) const { Name->output(OS, Flags); }

void VcallThunkIdentifierNode::output(string_accum& OS, OutputFlags Flags) const { OS << "`vcall'{" << OffsetInVTable << ", {flat}}"; }

void SpecialTableSymbolNode::output(string_accum& OS, OutputFlags Flags) const {
