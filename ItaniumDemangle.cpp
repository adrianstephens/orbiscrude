...
struct DumpVisitor {
	...
	template<typename T, T* = (bool*)nullptr> void print(T B) { printStr(B ? "true" : "false"); }
	void										   print(size_t N) { fprintf(stderr, "%zu", N); }
...
class BumpPointerAllocator {
	...
	void grow() {
		char* NewMeta = static_cast<char*>(malloc(AllocSize));
		if (NewMeta == nullptr)
			terminate();
	...
	void* allocateMassive(size_t NBytes) {
		...
		BlockMeta* NewMeta = reinterpret_cast<BlockMeta*>(malloc(NBytes));
		if (NewMeta == nullptr)
			terminate();
...
public:
	...
	template<typename T, typename... Args> T* makeNode(Args&&... args) { return new (Alloc.allocate(sizeof(T))) T(forward<Args>(args)...); }
...
char* llvm::itaniumDemangle(const char* MangledName, char* Buf, size_t* N, int* Status) {
	...
	Demangler	Parser(MangledName, MangledName + strlen(MangledName));
	string_accum S;
...
bool llvm::itaniumFindTypesInMangledName(const char* MangledName, void* Ctx, void (*Callback)(void*, const char*)) {
	Demangler Parser(MangledName, MangledName + strlen(MangledName));
