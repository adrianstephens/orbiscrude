...
#define	S D3D12_STREAM_OUTPUT_DESC
	_MAKE_FIELD_IDX(S, 0, pSODeclaration),
	_MAKE_FIELD(S,NumEntries)
	_MAKE_FIELD_IDX(S, 2, pBufferStrides),
	_MAKE_FIELD(S,NumStrides)
	_MAKE_FIELD(S,RasterizedStream)
	TERMINATOR
};
...
#define	S D3D12_DEPTH_STENCIL_DESC1
	...
	CALL_FIELD(FrontFace),
	CALL_FIELD(BackFace),
	_MAKE_FIELD(S,DepthBoundsTestEnable)
	TERMINATOR
};
...
template<> field fields<D3D12_MIP_REGION>::f[] = {
	_MAKE_FIELDS(D3D12_MIP_REGION, Width, Height, Depth)
};
...
struct xD3D12_ROOT_SIGNATURE_DESC {
	UINT NumParameters;
	counted<const D3D12_ROOT_PARAMETER,0> pParameters;
	UINT NumStaticSamplers;
	counted<const D3D12_STATIC_SAMPLER_DESC,2> pStaticSamplers;
	D3D12_ROOT_SIGNATURE_FLAGS Flags;
};


template<> field fields<D3D12_ROOT_SIGNATURE_DESC>::f[]	= {
	_MAKE_FIELD_IDX(xD3D12_ROOT_SIGNATURE_DESC, 0, NumParameters),
	_MAKE_FIELD_IDX(xD3D12_ROOT_SIGNATURE_DESC, 1, pParameters),
	_MAKE_FIELD_IDX(xD3D12_ROOT_SIGNATURE_DESC, 2, NumStaticSamplers),
	_MAKE_FIELD_IDX(xD3D12_ROOT_SIGNATURE_DESC, 3, pStaticSamplers),
	_MAKE_FIELD_IDX(xD3D12_ROOT_SIGNATURE_DESC, 4, Flags),
	TERMINATOR
};
...
MAKE_PREFIXED_ENUMS(D3D12_FEATURE, D3D12_FEATURE_,
	D3D12_OPTIONS,
	ARCHITECTURE,
	FEATURE_LEVELS,
	FORMAT_SUPPORT,
...
field fields<D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_PARAMETERS>::f[]
		= {_MAKE_FIELDS(D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_PARAMETERS, pSrcResource, pDstResource, SubresourceCount, _Field_size_full_(SubresourceCount) pSubresourceParameters, Format, ResolveMode, PreserveResolveSource)};
...
template<> field fields<D3D12_RAYTRACING_GEOMETRY_DESC>::f[] = {_MAKE_FIELDS(
...
template<> field fields<D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS>::f[] = {_MAKE_FIELDS(
