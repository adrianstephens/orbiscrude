...
%token <sc_token>	ASSIGNAND_SY
%token <sc_token>	ASSIGNOR_SY
%token <sc_token>	ASSIGNXOR_SY
...
template_decl_header:	template_params STRUCT_SY struct_identifier
							{ $$ = TemplateHeader(cg, cg->tokenLoc, cg->current_scope, $3, $1); }
					|	template_params STRUCT_SY struct_identifier ':' TYPEIDENT_SY
							{ $$ = AddStructBase(TemplateHeader(cg, cg->tokenLoc, cg->current_scope, $3, $1), LookUpTypeSymbol(cg, $5)); }

template_params:		TEMPLATE_SY '<'
							{ cg->current_scope->formal++; }
						template_param_list '>'
							{ cg->current_scope->formal--; $$ = $4; }
...
templated_type:			TEMPLATEIDENT_SY
					...
					|	TEMPLATEIDENT_SY {} '<' {} template_arg_list {} '>'
							{ $$ = InstantiateTemplate(cg, cg->tokenLoc, cg->current_scope, LookUpTypeSymbol(cg, $1), $5); }
...
template_arg:			type_specifier
					|	additive_expression
							{ $$ = IntToType(cg, GetConstant(cg, $1, 0)); }
...
expression_statement2:	postfix_expression /* basic_variable */ '=' expression
					...
					|	postfix_expression ASSIGNAND_SY expression
							{ $$ = NewCompoundAssignmentStmt(cg, cg->tokenLoc, ASSIGNAND_OP, $1, $3); }
					|	postfix_expression ASSIGNOR_SY expression
							{ $$ = NewCompoundAssignmentStmt(cg, cg->tokenLoc, ASSIGNOR_OP, $1, $3); }
					|	postfix_expression ASSIGNXOR_SY expression
							{ $$ = NewCompoundAssignmentStmt(cg, cg->tokenLoc, ASSIGNXOR_OP, $1, $3); }
;
