...
#include "D:\dev\orbiscrude\shared\cgc\parser.h"
...
int YYPARSE_DECL() {
	...
	for (;;) {
		...
		while (yyresult == 0 && !(yyn = yydefred[yystate])) {
			if (token < 0) {
				token = (yytokentype)yylex(&yylval YYLEX_ARGS);
				if (token < 0)
					token = YYEOF;
...
yyerrlab:			++yynerrs;
				...
				if (yyerrflag < 3) {
					...
					while (!(yyn = yy_index(yysindex[yystate], YYERRCODE))) {
						...
						YYDESTRUCT_CALL("error: discarding state", yystack.top_state(), &yystack.getl(0));
						yystack.pop(1);
...
int yyreduce(YYSTACKGLR::STACK &yystack, int rule, int len, YYSTYPE &yyval YYPARSE_PARAMS) {
	if (len)
	...
	while (!yyresult) {
		...
		while (!(yyresult = yystack.reserve(64) ? 0 : YY_OOM)) {
			...
			while ((n = yydefred[state]) ||
				...
				if (n) {
					...
					YYSTYPE yyval;
					yyresult = yyreduce(*s, n, len, yyval YYPARSE_ARGS);
		...
		while (!yyresult) {
			...
			while (red_queue.empty()) {
				...
				int			state	= yyreduce(*s, r.n, r.len, yyval YYPARSE_ARGS);
...
static bool yyrecoverSyntaxError(yyGLRStack* yystackp, int yychar, YYSTYPE yylval, CG* cg) {
	...
	if (yystackp->yyerrState == 3)
		...
		while (true) {
			...
			yychar = yylex(&yylval YYLEX_ARGS);
			if (yychar <= YYEOF) {
				yychar = token = YYEOF;
				YYDPRINTF((stderr, "Now at end of input.\n"));
			} else {
...
yyacceptlab		yyresult = 0;
			...
			case yyerr:
yyuser_error:	if (yyrecoverSyntaxError(&yystack, yychar, yylval, cg)) {
					yyposn = yystack.tops[0]->yyposn;
					continue;
				}
			case yyoom:
yyexhaustedlab:	yyerror(cg, ("memory exhausted"));
				yyresult = 2;
				break;
		}
		break;
