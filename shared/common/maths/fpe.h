...
template<typename T> static constexpr T eps = T(0.5) / (uint64(1) << num_traits<T>::mantissa_bits);
...
template<typename T, typename P = epoly<>> struct fpe {
	...
	constexpr T	error() const	{ return P::template value<T> * e; }
};
...
template<typename P> struct fpe<void, P> {
	template<typename T> constexpr T	error() const	{ return P::template value<T>; }
};
...
template<typename A, typename B> struct cz_s {
	decltype(declval<A>() + declval<B>())	x;
	cz_s(A i, B j) : x(i + j) {}
	CZ		get()	const	{ return cz(x); }
	auto	val()	const	{ return x; }
};

template<typename T, typename Pi, typename Pj> struct cz_s<fpe<T, Pi>, fpe<T, Pj>> {
	static constexpr auto	Pi1 = epoly<1,2,1>() * epoly<1,1,1,1>() * Pi();
	static constexpr auto	Pj1 = epoly<1,2,1>() * epoly<1,1,1,1>() * Pj();
	T	x, ei, ej;

	cz_s(fpe<T, Pi> i, fpe<T, Pj> j) : x(i.x + j.x), ei(i.e), ej(j.e) {}
	template<typename U> CZ	get(U u)	const { return cz(u, Pi1.template value<U> * ei + Pj1.template value<U> * ej); }
	CZ						get()		const { return get(x); }
};

template<typename Pi, typename Pj> struct cz_s<fpe<void, Pi>, fpe<void, Pj>> {
	static constexpr auto	P1 = max_epoly(
		epoly<1,2,1>() * epoly<1,1,1,1>() * Pi(),
		epoly<1,2,1>() * epoly<1,1,1,1>() * Pj()
	);
	cz_s(fpe<void, Pi> i, fpe<void, Pj> j) {}
};

template<typename A, typename B> auto add_cz(A i, B j) {
	return cz_s<A, B>(i, j);
}

template<typename A, typename B> auto sub_cz(A i, B j) {
	return cz_s<A, B>(i, -j);
}
...
template<typename T, int N> struct fp {
	...
	template<int N2> fp(const fpv<T, N2> &b);
...
template<typename T> constexpr auto	MAX_COMPONENTS		= (1 << num_traits<T>::exponent_bits) / num_traits<T>::mantissa_bits + 1;
...
//fast add/sub requires |a| > |b| (or a == 0)
...
/*
//original:
template<typename T> constexpr fp<T,2> split0(T a) {
	constexpr T splitter		= (1 << ((num_traits<T>::mantissa_bits + 2) / 2)) + 1;
	T	c		= T(splitter * a);
	T	abig	= T(c - a);
	T	ahi		= c - abig;
	return {a - ahi, ahi};
...
template<typename T> constexpr fp<T,2> split1(T a, T ahi) {
	return {a - ahi, ahi};
}

template<typename T> constexpr fp<T,2> split(T a) {
	typedef	uint_for_t<T>	I;
	constexpr int	B	= (num_traits<T>::mantissa_bits + 2) / 2;
	return split1(a, force_cast<T>(((I&)a + bit<I>(B)) & ~bits(B)));
}
...
template<typename T> constexpr fp<T,1>	operator<<(const fp<T,1> &a, int b)	{ return a.t * T(1 << b); }
template<typename T> constexpr fp<T,1>	operator>>(const fp<T,1> &a, int b)	{ return a.t / T(1 << -b); }
template<typename T> constexpr fp<T,1>	operator*(fp<T,1> a, decltype(two))	{ return a.t * two; }

//-----------------------------------------------------------------------------
//	unrolled sums for N < 8
//-----------------------------------------------------------------------------

template<int N2, typename T, int N> auto&	slice(fp<T, N> &x, int O)		{ return *(fp<T, N2>*)(x.t + O); }
template<int N2, typename T, int N> auto&	slice(const fp<T, N> &x, int O)	{ return *(const fp<T, N2>*)(x.t + O); }
template<int N2, typename T, int N> auto&	low(const fp<T, N> &x)			{ return slice<N2>(x, 0); }
template<int N2, typename T, int N> auto&	high(const fp<T, N> &x)			{ return slice<N2>(x, N - N2); }

template<typename T, int... N> auto join(const fp<T, N>&...x) {
	fp<T, meta::reduce_add<N...>>	r;
	int	i = 0;
	int	dummy[] = {((slice<N>(r, i) = x), i += N)...};
	return r;
}
...
template<typename T> constexpr bool unsigned_gt(T a, T b) {
	return (a > b) == (a > -b);
}

template<typename T> constexpr T unsigned_max(T a, T b) {
	return unsigned_gt(a, b) ? a : b;
}
template<typename T> constexpr T unsigned_min(T a, T b) {
	return unsigned_gt(a, b) ? b : a;
}

// eliminate zeros
template<typename T> int expansion_zeroelim(int N, const T *a, T *h) {
	int	j = 0;
	for (int i = 0; i < N; ++i) {
		if (a[i])
			h[j++] = a[i];
...
template<typename T> T estimate_expansion(int N, const T *a) {
	T	Q = 0;
	for (int i = 0; i < N; i++)
		Q += a[i];
	return Q;
...
template<typename T> int sum_expansion(int NA, const T *a, int NB, T *b, T *h) {
	for (int i = 0; i < NB; ++i) {
...
// maintains the nonoverlapping property; a and h can be the same, but b and h cannot
template<typename T> int sum_expansion_zeroelim(int NA, const T *a, int NB, const T *b, T *h) {
	for (int i = 0; i < NB; ++i) {
...
template<typename T> int fast_sum_expansion(int NA, const T *a, int NB, const T *b, T *h) {
	T	*h0		= h;
	int	ai		= 0,	bi		= 0;
	T	anow	= a[0],	bnow	= b[0];
...
// Sum two expansions, eliminating zero components
// if round-to-even is used, maintains the strongly nonoverlapping property; does NOT maintain the nonoverlapping or nonadjacent properties; h cannot be a or b
template<typename T> int fast_sum_expansion_zeroelim(int NA, const T *a, int NB, const T *b, T *h) {
	T	*h0		= h;
	int	ai		= 0,	bi		= 0;
	T	anow	= a[0],	bnow	= b[0];
...
template<typename T> int linear_sum_expansion(int NA, const T *a, int NB, const T *b, T *h) {
	int ai		= 0,	bi		= 0;
	T	anow	= a[0],	bnow	= b[0];
...
// Sum two expansions, eliminating zero components
// maintains the nonoverlapping property; h cannot be a or b
template<typename T> int linear_sum_expansion_zeroelim(int NA, const T *a, int NB, const T *b, T *h) {
	int ai		= 0,	bi		= 0;
	T	anow	= a[0],	bnow	= b[0];
...
template<typename T> int mul_expansion(int NA, const T *a, int NB, const T *b, T *h, T *scratch) {
	if (NB == 1)
		return scale_expansion(NA, a, *b, h);
	int	n0 = mul_expansion(NA, a, NB / 2, b, scratch, h);
	int	n1 = mul_expansion(NA, a, NB - NB / 2, b + NB / 2, scratch + n0, h);
	return fast_sum_expansion(n0, scratch, n1, scratch + n0, h);
}

template<typename T> int mul_expansion_zeroelim(int NA, const T *a, int NB, const T *b, T *h, T *scratch) {
	if (NB == 1)
		return scale_expansion_zeroelim(NA, a, *b, h);
	int	n0 = mul_expansion_zeroelim(NA, a, NB / 2, b, scratch, h);
	int	n1 = mul_expansion_zeroelim(NA, a, NB - NB / 2, b + NB / 2, scratch + n0, h);
	return fast_sum_expansion_zeroelim(n0, scratch, n1, scratch + n0, h);
}

template<typename T> int square_expansion(int N, const T *a, T *h, T *scratch) {
	if (N == 1) {
		*(fp<T,2>*)h = extend_square(split(*a), square(*a));
		return 2;
	}

	int	n0	= square_expansion(N / 2, a, h, scratch);
	int	n2	= square_expansion(N - N / 2, a + N / 2, h + n0, scratch);
	int	n02	= fast_sum_expansion(n0, h, n2, h + n0, scratch);

	int	n1	= scale_expansion_pow2(mul_expansion(N / 2, a, N - N / 2, a + N / 2, scratch + n02, h), scratch + n02, T(2), scratch + n02);
	return fast_sum_expansion(n02, scratch, n1, scratch + n02, h);
}

template<typename T> int square_expansion_zeroelim(int N, const T *a, T *h, T *scratch) {
	if (N == 1) {
		*(fp<T,2>*)h = extend_square(split(*a), square(*a));
		if (h[0])
			return 2;
	...
	int	n0	= square_expansion_zeroelim(N / 2, a, h, scratch);
	int	n2	= square_expansion_zeroelim(N - N / 2, a + N / 2, h + n0, scratch);
	int	n02	= fast_sum_expansion_zeroelim(n0, h, n2, h + n0, scratch);

	int	n1	= scale_expansion_pow2(mul_expansion_zeroelim(N / 2, a, N - N / 2, a + N / 2, scratch + n02, h), scratch + n02, T(2), scratch + n02);
	return fast_sum_expansion_zeroelim(n02, scratch, n1, scratch + n02, h);
}

template<typename T> no_inline int div_expansion(int NA, const T *a, T b, int NH, T *h, T *scratch, T *scratch2) {
	auto	bs	= split(b);
	T		*h1	= h + NH;
	while (h1 > h && NA) {
		T	q	= a[NA - 1] / b;
		*--h1	= q;
		if (h1 != h) {
			auto t	= -mul(q, b, bs);
			NA		= fast_sum_expansion_zeroelim<T>(NA, a, 2, t, scratch);
			a		= scratch;
...
template<typename T> no_inline int div_expansion_zeroelim(int NA, const T *a, T b, int NH, T *h, T *scratch, T *scratch2) {
	auto	bs	= split(b);
	T		*he	= h + NH, *h1 = he;
	while (h1 > h && NA) {
		T	q	= a[NA - 1] / b;
...
template<typename T> no_inline int div_expansion(int NA, const T *a, int NB, const T *b, int NH, T *h, T *scratch, T *scratch2) {
	T		*h1	= h + NH;
	while (h1 > h && NA) {
		T	q	= a[NA - 1] / b[NB - 1];
		*--h1	= q;
		if (h1 != h) {
			int	N	= negate_expansion(scale_expansion_zeroelim(NB, b, q, scratch), scratch, scratch);
			NA		= fast_sum_expansion_zeroelim(NA, a, N, scratch, scratch2);
			a		= scratch2;
		}
	}
...
template<typename T> no_inline int div_expansion_zeroelim(int NA, const T *a, int NB, const T *b, int NH, T *h, T *scratch, T *scratch2) {
	T		*he	= h + NH, *h1 = he;
	while (h1 > h && NA) {
		T	q	= a[NA - 1] / b[NB - 1];
		if (!q)
...
// Compress an expansion, possibly truncating
// Maintains the nonoverlapping property; a and h may be the same if NH >= NA
template<typename T> no_inline int compress_expansion(int NA, const T *a, int NH, T *h) {
	if (NA == 0)
		return 0;

	T		*h0 = h, *he = h + NH, *h1 = he;
	const T	*a1	= a + NA;
	fp<T,1> Q = *--a1;

	while (a1 > a) {
		auto S = add_fast<T>(Q, *--a1);
		if (S.lo) {
			if (h1 == h0) {
				Q = 0;
				break;
...
/*
template<typename T> int maybe_compress_expansion(int NA, const T *a, int NH, T *h) {
	if (NA > NH)
		return compress_expansion(NA, a, NH, h);
...
*/
template<typename T, int NA0, int NH> enable_if_t<(NA0 <= NH && NA0 <= 8), int> maybe_compress_expansion(int NA, const T *a, T *h) {
	for (int i = 0; i < NA0; i++)
		*h++ = a[i];
	return NA;
}
...
template<typename T, int NA0, int NH> enable_if_t<(NA0 > NH), int> maybe_compress_expansion(int NA, const T *a, T *h) {
	if (NA > NH)
		return compress_expansion(NA, a, NH, h);
...
//-----------------------------------------------------------------------------
//	multi-precision float operators & functions
//-----------------------------------------------------------------------------
...
template<typename T, int N> T estimate(const fp<T,N> &a) {
	return estimate_expansion<T>(N, a);
}
...
template<typename T, int NA, int NB> inline not_unrolled_t<T, NA, NB> operator+(const fp<T,NA> &a, const fp<T,NB> &b) {
	fp<T, NA + NB>	r;
	fast_sum_expansion<T>(NA, a, NB, b, r.t);
	return r;
}
...
template<typename T, int N, typename B> inline auto operator-(const fp<T,N> &a, const B &b) {
	return a + -b;
}
...
template<typename T, int N> inline auto&	operator<<=(fp<T,N> &a, int b)				{ scale_expansion_pow2<T>(N, a, T(1 << b), a.t); return a; }
template<typename T, int N> inline auto&	operator>>=(fp<T,N> &a, int b)				{ scale_expansion_pow2<T>(N, a, T(1) / T(1 << -b), a.t); return a; }
...
template<typename T, int N> constexpr auto	square(const fp<T,N> &a) {
	return square(a.lo) + ((a.lo * a.hi) << 1) + square(a.hi);
}
...
template<typename T, int N> struct fpv {
	...
	fpv()		: n(0)				{}
	fpv(T x)	: n(int(x != 0))	{ t[0] = x; }
	fpv(const fpv &x) = default;
	template<int N2> fpv(const fpv<T, N2> &x)									: n(maybe_compress_expansion<T, N2, N>(x.n, x, t))	{}
	template<int N2, typename = enable_if_t<(N2 <= N)>> fpv(const fp<T,N2> &x)	: n(expansion_zeroelim<T>(N2, x, t))			{}
	
	fpv&	operator=(T x)			{ t[0] = x; n = int(x != 0); return *this; }
	fpv&	operator=(fp<T,1> x)	{ return operator=(x.t); }

	template<int N2> fpv& operator=(const fpv<T,N2> &x) { n = maybe_compress_expansion<T, N2, N>(x.n, x.t, t); return *this; }
	template<int N2> fpv& operator=(const fp<T,N2> &x)	{ n = expansion_zeroelim<T>(N2, x, t); return *this; }

//	explicit operator bool()			const	{ return n != 0; }
	constexpr operator const T*()		const	{ return t; }
	T		top()						const	{ return n ? t[n - 1] : 0; }
	fpv&	compress()							{ n = compress_expansion(n, t, n, t); return *this; }

	fpv		operator-() const {
		fpv<T, N>	r;
		r.n = negate_expansion<T>(n, t, r.t);
		return r;
	}
	auto	operator+(fp<T,1> b) const {
		fpv<T, N + 1>	r;
		r.n = grow_expansion_zeroelim<T>(n, t, b, r.t);
		return r;
	}
	template<int NB> auto operator+(const fp<T,NB> &b)	const {
		fpv<T, N + NB>	r;
		r.n = fast_sum_expansion_zeroelim<T>(n, t, NB, b, r.t);
		return r;
	}
	template<int NB> auto operator+(const fpv<T,NB> &b)	const {
		fpv<T, N + NB>	r;
		r.n = fast_sum_expansion_zeroelim<T>(n, t, b.n, b.t, r.t);
		return r;
	}

	auto	operator*(fp<T,1> b) const {
		fpv<T, N + N>	r;
		r.n = scale_expansion_zeroelim<T>(n, t, b.t, r.t);
		return r;
	}

	template<int NB> inline auto operator*(const fpv<T,NB> &b) const {
		fpv<T, N * NB * 2>	r;
		T	scratch[N * NB * 2];
		r.n = mul_expansion_zeroelim(n, t, b.n, b.t, r.t, scratch);
		return r;
	}
	auto	operator/(fp<T,1> b) const {
		fpv	r;
		T	scratch[N * 2], scratch2[N * 2];
		r.n = div_expansion_zeroelim<T>(n, t, b.t, N, r.t, scratch, scratch2);
		return r;
	}
	template<int NB> auto	operator/(const fpv<T,NB> &b) const {
		fpv	r;
		T	scratch[N * 2], scratch2[N * 2];
		r.n = div_expansion_zeroelim<T>(n, t, b.n, b.t, N, r.t, scratch, scratch2);
		return r;
	}
	auto	operator+(T b)				const	{ return *this + fp<T,1>(b); }
	auto	operator*(T b)				const	{ return *this * fp<T,1>(b); }
	auto	operator/(T b)				const	{ return *this / fp<T,1>(b); }
	auto	operator*(decltype(two))	const	{ fpv<T,N> r; scale_expansion_pow2<T>(n, t, T(2), r.t); return r; }
	auto	operator<<(int b)			const	{ fpv<T,N> r; scale_expansion_pow2<T>(n, t, T(1 << b), r.t); return r; }
	auto	operator>>(int b)			const	{ fpv<T,N> r; scale_expansion_pow2<T>(n, t, T(1) / T(1 << b), r.t); return r; }

	template<typename B> auto operator-(const B &b)		const	{ return *this + -b; }

	auto&	operator+=(fpv<T,1> b)						{ n = grow_expansion_zeroelim<T>(n, t, b, t); return *this; }
	auto&	operator+=(T b)								{ return *this += fp<T,1>(b); }
	template<typename B> auto& operator+=(const B &b)	{ return *this = *this + b; }
	template<typename B> auto& operator-=(const B &b)	{ return *this += -b; }
	template<typename B> auto& operator*=(const B &b)	{ return *this = *this * b; }
	template<typename B> auto& operator/=(const B &b)	{ return *this = *this / b; }
	auto&	operator<<=(int b)							{ scale_expansion_pow2<T>(n, t, T(1 << b), t); return *this; }
	auto&	operator>>=(int b)							{ scale_expansion_pow2<T>(n, t, T(1) / T(1 << b), t); return *this; }

	auto&	operator++()								{ return *this += T(1); }
	auto&	operator--()								{ return *this -= T(1); }
	auto	operator++(int)								{ auto t = *this; *this += T(1); return t; }
	auto	operator--(int)								{ auto t = *this; *this -= T(1); return t; }

	template<typename B> friend int	compare(const fpv &a, const B &b)	{ return int(sign(a - b)); }

	template<typename B> bool	operator> (const B &b)	const	{ return compare(*this, b) >  0; }
	template<typename B> bool	operator>=(const B &b)	const	{ return compare(*this, b) >= 0; }
	template<typename B> bool	operator==(const B &b)	const	{ return compare(*this, b) == 0; }
	template<typename B> bool	operator!=(const B &b)	const	{ return compare(*this, b) != 0; }
	template<typename B> bool	operator<=(const B &b)	const	{ return compare(*this, b) <= 0; }
	...
	friend T estimate(const fpv &a)		{ return estimate_expansion<T>(a.n, a); }

	friend auto square(const fpv &a) {
		T	scratch[Nsquare<N>];
		fpv<T, Nsquare<N>>	r;
		r.n = square_expansion_zeroelim(a.n, a.t, r.t, scratch);
		return r;
	}
};

template<typename T, int N> template<int N2> force_inline fp<T, N>::fp(const fpv<T, N2>& b) {
	int	i = compress_expansion<T>(b.n, b, N, t);
	while (i < N)
		t[i++] = 0;
}

template<typename T> template<int N2> force_inline fp<T, 1>::fp(const fpv<T, N2>& b) {
	t = estimate(b);
}

template<typename T, int N> force_inline fpv<T,N> zero_elim(const fp<T, N> &a) {
	return a;
}

template<typename T, int NA, int NB> force_inline auto operator+(const fp<T,NA> &a, const fpv<T,NB> &b) {
	fpv<T, NA + NB>	r;
	r.n = fast_sum_expansion_zeroelim<T>(NA, a, b.n, b.t, r.t);
	return r;
}
template<typename T, int NA, int NB> force_inline auto operator*(const fp<T,NA> &a, const fpv<T,NB> &b) {
	return b * a;
...
template<typename T, int NA, int NB> force_inline auto operator/(const fp<T,NA> &a, const fpv<T,NB> &b) {
	return zero_elim(a) / b;
}
template<typename A, typename T, int NB> force_inline auto operator/(A a, const fpv<T,NB> &b) {
	fpv<T, NB>	r;
	T	scratch[NB * 2], scratch2[NB * 2];
	T	a2	= a;
	r.n = div_expansion<T>(1, &a2, b.n, b.t, NB, r.t, scratch, scratch2);
	return r;
}
...
public:
	...
	template<int N2> fpv2(const fpv<T, N2> &x)									: t(scratch), n(maybe_compress_expansion<T, N2, N>(x.n, x, t)) {}
	template<int N2, typename = enable_if_t<(N2 <= N)>> fpv2(const fp<T,N2> &x)	: t(scratch), n(expansion_zeroelim<T>(N2, x, t)) {}

	fpv2&	operator=(T x)			{ t[0] = x; n = 1; return *this; }
	fpv2&	operator=(fp<T,1> x)	{ t[0] = x.t; n = 1; return *this; }
	template<int N2> fpv2&	operator=(const fpv<T,N2> &x)	{ n = maybe_compress_expansion<T, N2, N>(x.n, x, t); return *this; }
	template<int N2> fpv2&	operator=(const fp<T,N2> &x)	{ n = expansion_zeroelim<T>(N2, x, t); return *this; }

	fpv2&	operator+=(fpv<T,1> b) {
		if (n == N)
			n = compress_expansion(n, t, n, t);
	...
	template<int NB> fpv2& operator+=(const fpv<T,NB> &b) {
		if (b.n) {
			...
			T	*t0 = t;
			t = t0 == scratch ? scratch + N : scratch;
			n = fast_sum_expansion_zeroelim<T>(n, t0, b.n, b.t, t);
		}
		return *this;
	}
	template<int NB> fpv2& operator+=(const fp<T,NB> &b) {
		...
		T	*t0 = t;
		t = t0 == scratch ? scratch + N : scratch;
		n = fast_sum_expansion_zeroelim<T>(n, t0, NB, b.t, t);
		return *this;
	}
	...
	constexpr operator const T*()	const	{ return t; }
	T		top()					const	{ return t[n - 1]; }
	fpv2	compress()						{ n = compress_expansion(n, t, n, t); return *this; }

	friend T estimate(const fpv2 &a)	{ return estimate_expansion<T>(a.n, a.t); }
...
template<typename T, int N> string_accum& print(string_accum &a, fpv<T, N> &&val, int precision = 0) {
	constexpr auto	M			= num_traits<T>::mantissa_bits;
	typedef	uint_bits_t<M>		I;
	constexpr int	max_digits	= klog<10, (uint64(1) << M)>;
	constexpr int	max_prec	= FLOG_BASE(10, 2, num_traits<T>::exponent_bits - 1);
	...
	int		dp		= get_exponent<10>(estimate(val)) + 1;

	if (dp >= 0
		? (val >= pow(fpv<T,8>(10), uint32(dp)))
		: (val *  pow(fpv<T,8>(10), uint32(-dp)) > 1)
	)
		++dp;

	if (!precision)
		precision = max_prec + dp;

	if (dp <= 0)
		a << "0.";

	while (val.n && dp > max_digits) {
		dp -= max_digits;

		auto	scale	= pow(T(10), dp);
		auto	scale2	= pow(fpv<T,8>(10), uint32(dp));
		auto	i		= I(estimate(val) / scale);
		...
		if (val < 0) {
			--i;
			val += scale2;
		} else if (val.top() >= scale / 2) {
			auto	test = val - scale2;
			if (test >= 0) {
				++i;
				val = test;
			}
		...
		int		n		= max_digits;
		if (precision < n) {
			i = div_round(i, pow(I(10), uint32(n - precision)));
			n = precision;
		}

		put_num_base<10>(a.getp(n), n, i);
		precision -= max_digits;
	}

	while (val.n && precision > 0) {
		T		scale	= pow(T(10), max_digits - max(dp, 0));
		val *= scale;
		auto	i	= I(estimate(val));
		val -= T(i);
		...
		if (dp > 0) {
			n	= dp + 1;
			auto	p	= a.getp(n);
			put_num_base<10>(p, dp, i / pow(I(10), uint32(max_digits - dp)));
			p[dp] = '.';
			n	= max_digits - dp;
		}
...
template<typename T, int N> string_accum& operator<<(string_accum &a, const fpv<T, N> &x) {
	return print(a, fpv<T, MAX_COMPONENTS<T>>(x));
}
template<typename T, int N> string_accum& operator<<(string_accum &a, const fp<T, N> &x) {
	return a << zero_elim(x);
}

template<typename T, int N> inline size_t from_string(const char *s, const char *e, fpv<T,N> &t) {
	auto	s0	= s;
	char	c;
	t	= 0;

	while (s != e && (is_digit(c = *s++) || c == '\'')) {
		if (c != '\'')
			t = t * 10 + from_digit(c);
	}
...
template<typename T, typename F> auto	to_fpv(F f) {
	constexpr	int	N = div_round_up(num_traits<F>::mantissa_bits, num_traits<T>::mantissa_bits);
	fpv<T, N>	r;
	while (r.n < N && f) {
		T	t = (T)f;
		f	-= t;
...
//auto operator"" _fv(long double d)	{ return to_fpv<float>(d); }
auto operator"" _fv(const char *s)		{ return from_string<fpv<float, MAX_COMPONENTS<float>>>(s); }
//template<char... C> auto	operator"" _fv2()	{return constant_int<int_reader<C...>::value>();}
...
template<typename T> struct orient2d_s {
	static auto f(T ax, T ay, T bx, T by, T cx, T cy) {
		return sub_cz(
			(ax - cx) * (by - cy),
			(ay - cy) * (bx - cx)
		);
...
template<template<typename> class F, typename...P> int predicate(P...p) {
	auto	r	= F<fpe<float>>::f(p...);
	if (auto cz = r.get())
		return cz;

	auto	r2	= F<double>::f(p...);
	if (auto cz = r.get(r2.x))
		return cz;

	auto	r3 = F<fpv<float, 1>>::f(p...);
	return r3.get();
}
...
template<typename T> int orient3d(T *_pa, T *_pb, T *_pc, T *_pd) {
	...
	if (auto cz = add_cz(adet + bdet, cdet).get())
		return cz;
...
template<typename T> int incircle(T *_pa, T *_pb, T *_pc, T *_pd) {
	...
	if (auto c = add_cz(adet + bdet, cdet).get())
		return c;
