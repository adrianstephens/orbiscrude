...
public:
	...
	template<size_t N> StringView(const char (&Str)[N]) : First(Str), Last(Str + N - 1) {}
	StringView(const char* First_, const char* Last_) : First(First_), Last(Last_) {}
	StringView(const char* First_, size_t Len) : First(First_), Last(First_ + Len) {}
	StringView(const char* Str) : First(Str), Last(Str + strlen(Str)) {}
	StringView() : First(nullptr), Last(nullptr) {}

	StringView substr(size_t From) const { return StringView(begin() + From, size() - From); }

	size_t find(char C, size_t From = 0) const {
		size_t FindBegin = min(From, size());
	...
	bool startsWith(StringView Str) const {
		if (Str.size() > size())
			return false;
		return equal(Str.begin(), Str.end(), begin());
	}
	...
	friend string_accum& operator<<(string_accum &a, const StringView &s) {
		return a.merge(s.First, s.Last);
	}
};

inline bool operator==(const StringView& LHS, const StringView& RHS) { return LHS.size() == RHS.size() && equal(LHS.begin(), LHS.end(), RHS.begin()); }

namespace llvm {
namespace ms_demangle {

// This memory allocator is extremely fast, but it doesn't call dtors for allocated objects. That means you can't use STL containers (such as vector) with this allocator. But it pays off -- the demangler is 3x faster with this allocator compared to one with STL containers.
...
class ArenaAllocator {
	...
	void addNode(size_t Capacity) {
		...
		NewHead->Buf		   = new uint8[Capacity];
		NewHead->Next		   = Head;
...
public:
	...
	template<typename T, typename... Args> T* alloc(Args&&... ConstructorArgs) {
		...
		Head->Used += Size + Adjustment;
		if (Head->Used < Head->Capacity)
			return new (PP) T(forward<Args>(ConstructorArgs)...);

		addNode(AllocUnit);
		Head->Used = Size;
		return new (Head->Buf) T(forward<Args>(ConstructorArgs)...);
	}
...
// Storage classes
enum Qualifiers : uint8 { Q_None = 0, Q_Const = 1 << 0, Q_Volatile = 1 << 1, Q_Far = 1 << 2, Q_Huge = 1 << 3, Q_Unaligned = 1 << 4, Q_Restrict = 1 << 5, Q_Pointer64 = 1 << 6 };
...
enum class ReferenceKind : uint8 { None, LValueRef, RValueRef };
...
enum class IntrinsicFunctionKind : uint8 {
	None,
	New,						 // ?2 # operator new
...
struct TypeNode : public Node {
	...
	virtual void outputPost(string_accum& OS, OutputFlags Flags) const = 0;
...
struct ArrayTypeNode : public TypeNode {
	...
	void outputDimensionsImpl(string_accum& OS, OutputFlags Flags) const;
	void outputOneDimension(string_accum& OS, OutputFlags Flags, Node* N) const;

	// A list of fixed_array dimensions.  e.g. [3,4,5] in `int Foo[3][4][5]`
	NodeArrayNode* Dimensions = nullptr;

	// The type of fixed_array element.
...
struct NodeArrayNode : public Node {
	...
	void output(string_accum& OS, OutputFlags Flags, StringView Separator) const;
...
struct TemplateParameterReferenceNode : public Node {
	...
	fixed_array<int64, 3> ThunkOffsets;
	PointerAffinity		   Affinity		   = PointerAffinity::None;
